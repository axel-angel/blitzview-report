...
As we said, Views are non-strict collections and they guarantee operations are in {\it constant} time and {\it constant} memory for transformer functions.
This is possible because the View, the proxy that contains the underlying collection which was converted into a View, contains the transformers that the programmer requested until now.
As the computations are bookmarked into the view internals, no change is made to the inner collection, but only in the proxy which can be mutable or not.
...
Let's take an example to illustrate with the current Scala Views:
...
These very nice properties are important because they increase the possible use cases.
...
In our design, there are three types of transformers: \verb|Map| which change the type, \verb|Filter| which can drop elements according to a predicate and \verb|Identity| used at the bottom of the stack.
