...
We support an interesting subset of Scala Blitz collections (see above) but we decided to evict \verb|List|s as there cannot be used efficiently in parallel context and it's easy for users to convert them to Array.
...
The ``proof-performer'' given a proof returns an ephemeral instance of \verb|Viewable| whose sole purpose is to augment the collection with the \verb|bview| method.
...
The users already familiar with Scala Views will notice the only difference is \verb|bview| instead of \verb|view|.
...
The scheduler is an implicit that can be imported or explicitly specified to the View methods.
